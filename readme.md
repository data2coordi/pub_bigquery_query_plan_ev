...
セオリー通り絞り込みが強いstr_col5が条件指定されたテーブルPが駆動表となっている。
...
カーディナリティは低くてもstr_col4での絞り込みから駆動している。
...
実は、updateでデータを更新した直後は異常値指定してもstr_col5の指定が
駆動表とするクエリプランであった。
update実行から1分程度経過してから上記のように異常値の指定が考慮されはじめた。
データ更新後、1分以内には統計情報を収集していると思われる。
